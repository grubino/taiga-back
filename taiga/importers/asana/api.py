...
class AsanaImporterViewSet(viewsets.ViewSet):
    ...
    def auth_url(self, request, *args, **kwargs):
        ...
        (oauth_token, oauth_secret, url) = AsanaImporter.get_auth_url()
    ...
    def authorize(self, request, *args, **kwargs):
        ...
        try:
            oauth_data = request.user.auth_data.get(key="asana-oauth")
            oauth_token = oauth_data.extra['oauth_token']
            oauth_secret = oauth_data.extra['oauth_secret']
            oauth_verifier = request.DATA.get('code')
            oauth_data.delete()
            asana_token = AsanaImporter.get_access_token(oauth_token, oauth_secret, oauth_verifier)['oauth_token']
